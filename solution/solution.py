...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # Determine relevance of each chunk
        relevances = []
        for chunk in contexts:
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"You are an evaluation judge. Score the following AI answer on each criterion (0.0 to 1.0).\n\n"
            f"Question: {question}\n\n"
            f"Answer: {answer}\n\n"
            f"Rubric:\n{rubric_text}\n\n"
            f"Return a JSON object mapping each criterion name to a float score 0-1.\n"
        )

        reasoning = self.judge_llm_fn(prompt)

        # Try to parse scores from the LLM response
        try:
            scores = json.loads(reasoning)
            # Validate: must be dict of str -> float
            if isinstance(scores, dict) and all(isinstance(v, (int, float)) for v in scores.values()):
                return {"scores": {k: float(v) for k, v in scores.items()}, "reasoning": reasoning}
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            ctx = pair.retrieved_contexts if pair.retrieved_contexts else None
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        # Average retrieval metrics (only non-None values)
        ...
        avg_ctx_recall = sum(recall_vals) / len(recall_vals) if recall_vals else None
        avg_ctx_precision = sum(precision_vals) / len(precision_vals) if precision_vals else None
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if scores[min_metric] == scores.get("faithfulness") == scores.get("relevance") == scores.get("completeness"):
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Improve prompt engineering to better address user questions"
            )
        ...
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Add intent detection to identify and redirect off-topic queries"
            )

        # Ensure at least 3 suggestions
        default_suggestions = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement retrieval quality monitoring to catch context gaps early",
            "Add automated regression testing to CI/CD pipeline as quality gate",
        ]
